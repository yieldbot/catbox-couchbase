...
module.exports = function(options) {
  ...
  var keepAlive = function keepAlive() {
    ...
    keepAliveTimer = setInterval(function() {
      ...
      if(!running) {
        running = true;
        eEmitter.emit('keepalive', util.format('checking connection... (connected=%s)', cacheClient.connected));

        // If it is not connected then
        if(!cacheClient.connected) {
          eEmitter.emit('keepalive', 'trying to connect...');
          cacheClient = openBucket();
          running = false;
        } else {
          // Get an item
          cacheClient.get('keepalive', function(err) {
            ...
            // If error code is not 13 (The key does not exist on the server) then
            if(err && err.code !== 13) {
              eEmitter.emit('keepalive', util.format('trying to connect... (%s)', err.code));
              cacheClient = openBucket();
            }
            else {
