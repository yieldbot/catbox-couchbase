...
module.exports = function(options) {
  ...
  if(!serverOptions.connectionTimeout) serverOptions.connectionTimeout = 60000; // default 5000
  if(!serverOptions.operationTimeout)  serverOptions.operationTimeout  = 50000; // default 2500
  ...
  var bypassCacheOnConnError = !!flags.bypassCacheOnConnError,
    keepAliveFlag          = (flags.keepAlive === false) ? false : true,
    keepAliveTimer;
  ...
  // Opens bucket
  var openBucket = function openBucket() {

    // NOTE: CB library creates new instance for each open bucket call.

    if(cacheClient && cacheClient.disconnected) {
      cacheClient.disconnected();
    }

    // Create instance
    cacheClient = cluster.openBucket(serverBucket);
    // Code 24: failed to connect to bucket
    ...
    cacheClient.on('error', function(err) {
      eEmitter.emit('error', {code: err.code, message: err.message});
    });
    ...
    keepAliveTimer = setInterval(function() {
      ...
      if(!running) {
        ...
        if(!cacheClient.connected) {
          eEmitter.emit('keepalive', util.format('trying to connect... (connected=%s)', cacheClient.connected));
          cacheClient = openBucket();
          running = false;
        } else {
          // Get an item
          cacheClient.get('keepalive', function(err) {
            // Code 23: Client-Side timeout exceeded for operation
            // Code 16: Generic network failure
            // If error code is not 13 (The key does not exist on the server) then
            if(err && err.code !== 13) {
              eEmitter.emit('keepalive', util.format('trying to connect... (connected=%s err=%s)', cacheClient.connected, err.code));
              cacheClient = openBucket();
            }
            else {
              eEmitter.emit('keepalive', 'connection is alive');
            }
            running = false;
          });
        }
  ...
  var get = function get(key, callback) {
    ...
    if(!cacheClient.connected && bypassCacheOnConnError) {
      eEmitter.emit('bypass', 'get ' + keyF + ' (c)');
    ...
    cacheClient.get(keyF, function(err, result) {
      if(err) {
        ...
        if(bypassCacheOnConnError && err.code === 16) {
          eEmitter.emit('bypass', 'get ' + keyF + ' (g)');
          return callback(null, null); // bypass cache
        }
        // If error code is not 13 (The key does not exist on the server) then
        if(err.code !== 13) {
  ...
  var set = function set(key, value, ttl, callback) {
    ...
    if(!cacheClient.connected && bypassCacheOnConnError) {
      eEmitter.emit('bypass', 'set ' + keyF + ' (c)');
    ...
    cacheClient.upsert(keyF, envelopeStr, {expiry: expiry}, function(err) {
      if(err) {
        ...
        if(bypassCacheOnConnError && err.code === 16) {
          eEmitter.emit('bypass', 'set ' + keyF + ' (u)');
          return callback(); // bypass cache
        }
        return callback(err);
      }
  ...
  var drop = function drop(key, callback) {
    ...
    if(!cacheClient.connected && bypassCacheOnConnError) {
      eEmitter.emit('bypass', 'drop ' + keyF + ' (c)');
      return callback(); // bypass cache
    }
    ...
    cacheClient.remove(keyF, function(err) {
      if(err) {
        // Code 16: failed to connect to bucket.
        if(bypassCacheOnConnError && err.code === 16) {
          eEmitter.emit('bypass', 'drop ' + keyF + ' (r)');
          return callback(); // bypass cache
        }
        // If error code is not 13 (The key does not exist on the server) then
        if(err.code !== 13) {
  ...
  if(flags.debug instanceof Array) {
    ...
    if(flags.debug.indexOf('events') !== -1) {
      // Log all the events
      ['start', 'stop', 'get', 'set', 'drop', 'connect', 'error', 'keepalive', 'bypass'].forEach(function(event) {
