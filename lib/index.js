...
module.exports = function(options) {
  ...
  var serverOptions    = options || {},
      serverLocations, // cluster locations
  ...
  // Server locations
  serverLocations = serverOptions.location || ((serverOptions.host || '127.0.0.1') + ':' + (serverOptions.port || 8091));
  serverLocations = (serverLocations instanceof Array) ? serverLocations.join(',') : serverLocations;

  // Server partition
  serverPartition = serverOptions.partition || 'default';
  ...
  var start = function start(callback) {
    ...
    bucket  = cluster.openBucket(serverPartition, function(err) {
      if(err) {
        throw new Error(util.format('bucket %s/pools/default/buckets/%s could not be opened', serverLocations, serverPartition));
      }

      // Set events
      bucket.on('connect', function() {
        eEmitter.emit('connect', util.format('connected to %s bucket', serverPartition));
      });
      bucket.on('error', function(err) {
        eEmitter.emit('error', err);
      });
      eEmitter.emit('start', util.format('started for %s/pools/default/buckets/%s', serverLocations, serverPartition));
  ...
  var stop = function stop() {
    if(bucket) {
      ...
      eEmitter.emit('stop', util.format('stopped for %s/pools/default/buckets/%s', serverLocations, serverPartition));
    }
  };
  ...
  var generateKey = function generateKey(key) {
    return (key && key.segment ? encodeURI(key.segment) : '') + (key && key.id ? ':' + encodeURI(key.id) : '');
  };
